...
def render_header(connected=True):
    ...
    # Logo del Departamento de Acuicultura - UCN
    import base64
    from pathlib import Path
    
    # Cargar y codificar el logo en base64
    logo_path = Path(__file__).parent.parent / "assets" / "Logo-Acuicultura.png"
    if logo_path.exists():
        with open(logo_path, "rb") as f:
            logo_data = base64.b64encode(f.read()).decode()
        logo_html = f'<img src="data:image/png;base64,{logo_data}" style="width: 56px; height: 56px; object-fit: contain; border-radius: 14px; background: rgba(255,255,255,0.95); padding: 4px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);" />'
    else:
        # Fallback al icono SVG si no se encuentra el logo
        logo_html = '''<div style="width: 56px; height: 56px; background: rgba(255,255,255,0.15); border-radius: 14px; backdrop-filter: blur(10px); display: flex; align-items: center; justify-content: center; border: 1px solid rgba(255,255,255,0.2);">
...
{logo_html}
<div>
<h1 style="margin: 0; font-size: 1.8rem; font-weight: 700; color: white; line-height: 1.1;">Monitor Biofloc</h1>
<p style="margin: 0.25rem 0 0 0; color: rgba(255,255,255,0.85); font-size: 0.95rem;">Lab. Cultivos Crustáceos - UCN</p>
</div>
</div>
